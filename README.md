...
| 20 | [**Yapay Zeka Asistanı Botu**](./20-Yapay-Zeka-Asistanı-Botu) | Flask ve Google Gemini API ile Alican Kaya'nın dijital ikiz asistanı | `Flask`, `google-generativeai` | ✅ |
...
### 📂 Proje #20 — Yapay Zeka Asistanı Botu

Flask web çatısı ve Google Gemini API kütüphanesi kullanılarak oluşturulmuş, Alican Kaya'nın projelerini, yeteneklerini ve özgeçmişini ziyaretçilere birinci şahıs ağzından tanıtan interaktif bir dijital ikiz asistanı uygulaması:

| Dosya | Açıklama |
|-------|----------|
| [**yapay_zeka_asistan_app.py**](./20-Yapay-Zeka-Asistanı-Botu/yapay_zeka_asistan_app.py) | Oturum tabanlı, çoklu model fallback destekli ve kurumsal talimatlı ana Flask uygulaması |
| [**yapay_zeka_asistan_app_Aciklamalari.ipynb**](./20-Yapay-Zeka-Asistanı-Botu/yapay_zeka_asistan_app_Aciklamalari.ipynb) | Uygulamanın mimarisini, oturum sınırlarını, rol tabanlı geçmiş formatlamasını ve model yedekleme (fallback) mantığını satır satır anlatan notebook dosyası |
| [**flask_rehber.ipynb**](./20-Yapay-Zeka-Asistanı-Botu/flask_rehber.ipynb) | Flask web çatısını, rotalama mantığını, session yönetimini ve Jinja2 şablon motorunu sıfırdan anlatan eğitim rehberi |
| [**gemini_api_rehber.ipynb**](./20-Yapay-Zeka-Asistanı-Botu/gemini_api_rehber.ipynb) | Google Gemini API'sinin kimlik doğrulama, model listeleme, generation config ayarları ve kota/hata yönetimini detaylandıran rehber |
| [**README.md**](./20-Yapay-Zeka-Asistanı-Botu/README.md) | Proje özel açıklamaları ve ekran görüntüsü |
| `public/` | Arayüz ekran görüntüsünü barındıran görsel klasörü |

**Öğrenilen Konular:**
- Flask ile web tabanlı yönlendirmeler (routes), GET/POST metotları yönetimi ve kullanıcı arayüzü entegrasyonu
- Flask Session yönetimi ile istemci tarafında şifrelenmiş cookie bazlı konuşma geçmişi (context) saklama
- Çerez boyut sınırına (4KB) takılmamak için konuşma geçmişini dinamik olarak sınırlama (son 20 mesaj)
- Google Gemini API (`google-generativeai`) ile kimlik doğrulama, model yapılandırması (temperature, top_p, top_k, max_output_tokens) ve sohbet oturumları yönetimi
- `system_instruction` kullanarak yapay zekaya kimlik, rol, davranış kuralları ve kurumsal bilgi tanımlama
- Dinamik model listeleme (`genai.list_models()`) ve hata/kota engellerine karşı otomatik model yedekleme (Model Fallback) algoritması
...
│
├── 📂 20-Yapay-Zeka-Asistanı-Botu/
│   ├── README.md                               # Proje açıklaması
│   ├── yapay_zeka_asistan_app.py               # Oturum tabanlı Flask asistanı
│   ├── yapay_zeka_asistan_app_Aciklamalari.ipynb # Notebook kod açıklamaları
│   ├── flask_rehber.ipynb                      # Flask eğitim rehberi
│   ├── gemini_api_rehber.ipynb                 # Gemini API eğitim rehberi
│   └── 📂 public/                              # Görsel materyaller klasörü
│       └── projess.png                         # Arayüz ekran görüntüsü
│
